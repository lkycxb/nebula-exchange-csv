...
import com.ubisectech.nebula.exchange.common.GraphProvider
import com.ubisectech.nebula.exchange.common.common.{Edges, KeyPolicy, Vertices}
...
abstract class ServerBaseWriter extends Writer {
      ...
      .map { vertex =>
        ...
        UPDATE_VERTEX_TEMPLATE.format(
          ...
          vertex.values
            .map { value =>
              val updateValue =
                UPDATE_VALUE_TEMPLATE.format(vertices.names(index), value)
              index += 1
              updateValue
            }
            .mkString(",")
  ...
  def toUpdateExecuteSentence(edgeName: String, edges: Edges): String = {
    edges.values
      .map { edge =>
        var index = 0
        val rank = if (edge.ranking.isEmpty) {
          0
        } else {
          edge.ranking.get
        }
        UPDATE_EDGE_TEMPLATE.format(
          ...
          edge.values
            .map { value =>
              val updateValue =
                UPDATE_VALUE_TEMPLATE.format(edges.names(index), value)
              index += 1
              updateValue
            }
            .mkString(",")
...
class NebulaGraphClientWriter(dataBaseConfigEntry: DataBaseConfigEntry,
  ...
  override def writeVertices(vertices: Vertices, ignoreIndex: Boolean = false): String = {
    ...
    if (rateLimiter.tryAcquire(rateConfig.timeout, TimeUnit.MILLISECONDS)) {
      ...
      if (result.isSucceeded) {
        LOG.info(
          s" write ${config.name}, batch size(${vertices.values.size}), latency(${result.getLatency}),statement:${statement}")
        return null
      }
      LOG.error(s"write vertex failed for ${result.getErrorMessage},statement:${statement}")
  ...
  override def writeEdges(edges: Edges, ignoreIndex: Boolean = false): String = {
    ...
    if (rateLimiter.tryAcquire(rateConfig.timeout, TimeUnit.MILLISECONDS)) {
      ...
      if (result.isSucceeded) {
        LOG.info(
          s" write ${config.name}, batch size(${edges.values.size}), latency(${result.getLatency}us),statement:${statement}")
        return null
      }
  ...
  override def writeNgql(ngql: String): String = {
    if (rateLimiter.tryAcquire(rateConfig.timeout, TimeUnit.MILLISECONDS)) {
      ...
      LOG.error(s"reimport ngql failed for ${result.getErrorMessage},ngql:${ngql}")
    } else {
