...
import com.ubisectech.nebula.exchange.common.Argument
import com.ubisectech.nebula.exchange.common.common.KeyPolicy
...
object Configs {
  ...
  def parse(configPath: String, variable: Boolean = false, param: String = ""): Configs = {
    ...
    val graphAddresses = nebulaConfig.getStringList("address.graph").asScala.toList
    val metaAddresses = nebulaConfig.getStringList("address.meta").asScala.toList

    val space = nebulaConfig.getString("space")
    val databaseEntry = DataBaseConfigEntry(graphAddresses, space, metaAddresses)
    ...
    if (tagConfigs.isDefined) {
      for (tagConfig <- tagConfigs.get.asScala) {
        ...
        LOG.info(s"Sink Config ${sinkConfig}")
        ...
        val vertexUdf = if (tagConfig.hasPath("vertex.udf")) {
          ...
          val cols: List[String] = tagConfig.getStringList("vertex.udf.oldColNames").asScala.toList
    ...
    if (edgeConfigs.isDefined) {
      for (edgeConfig <- edgeConfigs.get.asScala) {
        ...
        val srcUdf = if (edgeConfig.hasPath("source.udf")) {
          ...
          val cols: List[String] = edgeConfig.getStringList("source.udf.oldColNames").asScala.toList
        ...
        val dstUdf = if (edgeConfig.hasPath("target.udf")) {
          ...
          val cols: List[String] = edgeConfig.getStringList("target.udf.oldColNames").asScala.toList
  ...
  private[this] def dataSourceConfig(category: SourceCategory.Value,
    ...
    category match {
      case SourceCategory.NEBULA =>
        val name = config.getString("name") //tag or edge
        val graphAddresses = nebulaConfig.getStringList("address.graph").asScala.mkString(",")
        val metaAddresses = nebulaConfig.getStringList("address.meta").asScala.mkString(",")
        val space = nebulaConfig.getString("space")
        val user = nebulaConfig.getString("user")
        val pswd = nebulaConfig.getString("pswd")
        NebulaSourceConfigEntry(SourceCategory.NEBULA, name, graphAddresses, metaAddresses, space, user, pswd)
      case SourceCategory.SST =>
        FileBaseSourceConfigEntry(SourceCategory.SST, config.getString("path"))
      ...
      case SourceCategory.ORACLE =>
        OracleConfigEntry(
          ...
          config.getString("password"),
          getOrElse(config, "table", null),
          getOrElse(config, "sentence", null) //获取数据的select语句
        )
  ...
  private[this] def dataSinkConfig(category: SinkCategory.Value,
    ...
    category match {
      ...
      case SinkCategory.CSV => {
        ...
        val separator = if (nebulaConfig.hasPath("path.separator")) {
          Option(nebulaConfig.getString("path.separator"))
        } else {
          Option.empty
        }
