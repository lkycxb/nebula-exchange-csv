...
import com.ubisectech.nebula.exchange.common.config.{SslConfigEntry, SslType, Type}
import com.vesoft.nebula.PropertyType
import com.vesoft.nebula.client.graph.data.{CASignedSSLParam, HostAddress, SSLParam, SelfSignedSSLParam}
...
class MetaProvider(addresses: List[HostAddress],
  ...
  def getTagSchema(space: String, tag: String): Map[String, Integer] = {
    ...
    val schema = new mutable.LinkedHashMap[String, Integer]
  ...
  def getTagSchemaSeq(space: String, tag: String): Seq[(String, Int)] = {
    val tagSchema = metaClient.getTag(space, tag)
    val schema = new mutable.ArrayBuffer[(String, Int)]
    val columns = tagSchema.getColumns
    for (colDef <- columns.asScala) {
      schema += ((new String(colDef.getName), colDef.getType.getType.getValue))
    }
    schema
  }

  def getEdgeSchemaSeq(space: String, edge: String): Seq[(String, Int)] = {
    val edgeSchema = metaClient.getEdge(space, edge)
    val schema = new mutable.ArrayBuffer[(String, Int)]
    val columns = edgeSchema.getColumns
    for (colDef <- columns.asScala) {
